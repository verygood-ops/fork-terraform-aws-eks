...
| <a name="output_user_data_templates"></a> [user\_data\_templates](#output\_user\_data\_templates) | Map of built-in user\_data template paths, keyed by the OS family they bootstrap (al2, al2023, bottlerocket, windows). Useful for pointing `user_data_template_path` at the module's own template when using a custom AMI (ami\_type = "CUSTOM") that is otherwise compatible with one of these bootstrap formats, without having to vendor a copy of the template into the calling configuration. |
<!-- END_TF_DOCS -->
